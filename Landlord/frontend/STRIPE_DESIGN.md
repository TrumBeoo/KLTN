...
- Use professional color palette (white, gray, subtle blue accents)
- Use clear typography for readability
- Use strong layout structure for data-heavy UI
- Use spacing-based layout (8px grid system)
- Focus on clarity, precision, and usability
...
- Cards → clean statistic cards (metrics, KPIs)
- Tables → structured, readable, with clear columns
- Buttons → clear primary/secondary hierarchy
- Forms → structured, labeled, easy to scan
- Modals → functional, clear actions

For admin dashboard (CCMN):
- Show system metrics (users, rooms, revenue)
- Room management table
- Tenant management table
- Contract and payment tracking
- Upload job tracking (from your system)
- Moderation / approval panel

CRITICAL UX rules:
- Prioritize data clarity over visual style
- Make actions obvious and accessible
- Maintain consistent layout across pages
- Support heavy data interaction (filter, sort)

Technical constraints:
- Use reusable React components
- Keep responsive design (desktop-first, but mobile usable)
- Do NOT break existing functionality

Output:
- Refactored admin components
- Structured Stripe-style dashboard UI
- Consistent and professional system interface

Improve the UI to feel closer to Stripe dashboard experience.

Focus on:
- Better data hierarchy and readability
- Clear separation between sections
- Improved table design (alignment, spacing, clarity)
- More consistent use of typography and colors
- Better layout structure for complex data

Enhance UX:
- Make key metrics stand out clearly
- Improve scanning experience for tables and lists
- Highlight important actions (edit, approve, delete)
- Improve filter, sorting, and navigation clarity

Polish details:
- Align all columns and components precisely
- Ensure consistent spacing across sections
- Reduce unnecessary visual noise
- Improve responsiveness for large datasets

Do NOT:
- Change logic or data structure
- Remove important admin features
- Convert data-heavy UI into overly simplified UI

Goal:
- Professional, structured, and highly usable admin dashboard
- Clear, efficient, and scalable UI like Stripe