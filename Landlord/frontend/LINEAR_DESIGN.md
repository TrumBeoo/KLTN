...
- Use dark/light modern palette (neutral colors, subtle accents)
- Use smooth UI with soft shadows and rounded corners
- Maintain clean typography with clear hierarchy
- Use spacing-based layout (8px grid system)
- Focus on simplicity, speed, and fluid experience
- Add subtle animations and transitions (hover, loading)

Layout changes:
- Minimal top navigation or compact sidebar
- Main content centered, clean, and uncluttered
- Use sections with clear spacing instead of heavy borders

Components transformation:
- Cards → modern flat cards with subtle shadow and hover effect
- Buttons → rounded, minimal, smooth hover states
- Tables → simplified or converted into clean list views
- Forms → modern inputs with focus states and spacing
- Modals → smooth animation, centered, clean UI

For tenant dashboard (CCMN):
- Room info → modern card with image, price, and status
- Payment → clean card with clear highlight (paid/unpaid)
- Contract → structured section, easy to read
- Maintenance → simple, fast interaction form

CRITICAL UX rules:
- Keep UI fast and responsive
- Keep interactions smooth (like a real app)
- Do NOT overload UI with too many elements
- Maintain strong usability for mobile users
...
- Smooth, modern Linear-style UI
- Consistent design across all tenant pages

Improve the UI to feel closer to Linear product experience.

Focus on:
- Smoother interactions (hover, transitions, loading states)
- Better spacing and alignment between components
- More consistent typography hierarchy
- Subtle shadows and depth for modern feel
- Refined card layouts with better balance

Enhance UX:
- Make interactions feel fast and responsive
- Improve visual feedback (hover, active states)
- Ensure important actions are easy to access
- Optimize layout for both desktop and mobile

Polish details:
- Align elements precisely
- Reduce visual clutter
- Improve grouping of related content
- Make UI feel like a high-quality SaaS app
